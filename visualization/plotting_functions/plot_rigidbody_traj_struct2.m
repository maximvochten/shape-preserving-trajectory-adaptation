function plot_rigidbody_traj_struct2(R_demo,p_obj_demo,R_recon_struct,p_obj_recon_struct,R_FS_demo,R_FS_recon_struct,q,constraints_struct,title,paper_experiment_type,show_rotation,descriptor)

if isempty(R_recon_struct)    
    ...
    N2 = size(R_recon_struct{1},3);
     if isempty(constraints_struct)
         M = 1;
     else
        M = size(R_recon_struct,2);
     end
end
...
if ~isempty(R_FS_demo) || ~isempty(R_FS_recon_struct)
    inc = 25; 
else
...
if strcmp(paper_experiment_type,'jerkaccuracy') || strcmp(paper_experiment_type,'dmp') || strcmp(paper_experiment_type,'dmp2') || strcmp(paper_experiment_type,'robot')
    a=1;b=2;c=3;
    view_angles = [-119 ; 16];
    inc = 3;
else
...
if strcmp(paper_experiment_type,'refpoint')
    view_angles = [-126 ; 12];
end
if strcmp(paper_experiment_type,'target_pose2') || strcmp(paper_experiment_type,'initial_pose2') || strcmp(paper_experiment_type,'target_direction')
    inc = 3;
end
...
Rx_FS_demo = []; Ry_FS_demo = []; Rz_FS_demo = []; p_FS_demo = [];
for j=round(linspace(1,N,inc))
    ...
    if ~isempty(R_FS_demo)
        Rx_FS_demo = [Rx_FS_demo ; R_FS_demo(1:3,1,j)'];
        Ry_FS_demo = [Ry_FS_demo ; R_FS_demo(1:3,2,j)'];
        Rz_FS_demo = [Rz_FS_demo ; R_FS_demo(1:3,3,j)'];
        if strcmp(descriptor,'screw_axis')
            p_FS_demo = [p_FS_demo ; R_FS_demo(1:3,4,j)'];
        elseif strcmp(descriptor,'frenetserret')
            p_FS_demo = [p_FS_demo ; p_obj_demo(j,:)];
        end
    end
    if j==N
...
if ~isempty(R_FS_demo)
    arrow3(p_FS_demo,p_FS_demo+len*Rx_FS_demo,['_y' linewidth],t,2*t)
    arrow3(p_FS_demo,p_FS_demo+len*Ry_FS_demo,['_m' linewidth],t,2*t)
    arrow3(p_FS_demo,p_FS_demo+len*Rz_FS_demo,['_c' linewidth],t,2*t)
end

for m=1:M
    R_recon = R_recon_struct{m};
    p_obj_recon = p_obj_recon_struct{m};
    if ~isempty(R_FS_recon_struct)
        R_FS_recon = R_FS_recon_struct{m};
    end
    ...
    for j=round(linspace(1,length(p_obj_recon),inc))
        ...
        if ~isempty(R_FS_recon_struct)
            Rx_FS_recon = [Rx_FS_recon ; R_FS_recon(1:3,1,j)'];
            Ry_FS_recon = [Ry_FS_recon ; R_FS_recon(1:3,2,j)'];
            Rz_FS_recon = [Rz_FS_recon ; R_FS_recon(1:3,3,j)'];
            if strcmp(descriptor,'screw_axis')
                p_FS_recon = [p_FS_recon ; R_FS_recon(1:3,4,j)'];
            elseif strcmp(descriptor,'frenetserret')
                p_FS_recon = [p_FS_recon ; p_obj_recon(j,:)];
            end
        end
        
        if j==length(p_obj_recon)
        ...
        elseif j~=1 && j~=length(p_obj_recon)
            if show_rotation
    ...
    if ~isempty(R_FS_recon_struct)
        arrow3(p_FS_recon,p_FS_recon+0.5*len*Rx_FS_recon,['_y' linewidth],t,2*t)
        arrow3(p_FS_recon,p_FS_recon+0.5*len*Ry_FS_recon,['_m' linewidth],t,2*t)
        arrow3(p_FS_recon,p_FS_recon+0.5*len*Rz_FS_recon,['_c' linewidth],t,2*t)
    end
    
    if ~isempty(R_FS_recon_struct) && strcmp(descriptor,'screw_axis')
        plot3(offset(3)+p_FS_recon(:,a),offset(2)+p_FS_recon(:,b),offset(1)+p_FS_recon(:,c),'k.-','MarkerSize',15,'LineWidth',1.5);
        plot3(offset(3)+p_FS_recon(1,a),offset(2)+p_FS_recon(1,b),offset(1)+p_FS_recon(1,c),'ko','MarkerSize',5,'LineWidth',1.5);
        plot3(offset(3)+p_FS_recon(end,a),offset(2)+p_FS_recon(end,b),offset(1)+p_FS_recon(end,c),'kx','MarkerSize',15,'LineWidth',1.5);
    end
...
for m=1:M
    ...
    if show_rotation
        for j=round(linspace(1,N2,inc))
            ...
            vm = repmat(p_obj_recon(j,:)+offset',8,1) + [ R_recon(1:3,1:3,j)*[0; -lency/2; -lencz/2] ...
                R_recon(1:3,1:3,j)*[lencx; -lency/2; -lencz/2] R_recon(1:3,1:3,j)*[lencx; lency/2; -lencz/2] ...
                R_recon(1:3,1:3,j)*[0; lency/2; -lencz/2] R_recon(1:3,1:3,j)*[0; -lency/2; lencz/2] ...
                R_recon(1:3,1:3,j)*[lencx; -lency/2; lencz/2] R_recon(1:3,1:3,j)*[lencx; lency/2; lencz/2] ...
                R_recon(1:3,1:3,j)*[0; lency/2; lencz/2] ]';
            vm=vm(:,[a b c]);
            if j==1 && ~strcmp(paper_experiment_type,'target_pose')
                ...
                patch('Vertices',vm([2 3 7 6],:),'Faces',[1 2 3 4], 'EdgeAlpha',0.8,'FaceColor',[0.80 0.25 0.25],'FaceAlpha',0.40,'EdgeColor',[0.90 0.15 0.15],'LineWidth',0.75);
            elseif j==length(p_recon) && strcmp(paper_experiment_type,'initial_pose') && strcmp(paper_experiment_type,'initial_pose2')
            elseif j~=1
                patch('Vertices',vm,'Faces',fm, 'EdgeAlpha',0.8,'FaceColor',[0.80 0.25 0.25],'FaceAlpha',0.10,'EdgeColor',[0.90 0.15 0.15],'LineWidth',1.00);
                patch('Vertices',vm([2 3 7 6],:),'Faces',[1 2 3 4], 'EdgeAlpha',0.8,'FaceColor',[0.80 0.25 0.25],'FaceAlpha',0.40,'EdgeColor',[0.90 0.15 0.15],'LineWidth',0.75);
            end
    ...
    if strcmp(paper_experiment_type,'initial_pose') && strcmp(paper_experiment_type,'initial_pose2')
    ...
    if strcmp(paper_experiment_type,'adaptive')
        ...
        plot3(offset(3)+p_obj_recon(round(N2/2),a),offset(2)+p_obj_recon(round(N2/2),b),offset(1)+p_obj_recon(round(N2/2),c),'k.','MarkerSize',50);
    end
...
if show_rotation
    for j=round(linspace(1,N,inc))
        fm = [1 2 6 5; 2 3 7 6; 3 4 8 7; 4 1 5 8; 1 2 3 4; 5 6 7 8];
        vm = repmat(p_obj_demo(j,:),8,1) + [ R_demo(1:3,1:3,j)*[0; -lency/2; -lencz/2] ...
            R_demo(1:3,1:3,j)*[lencx; -lency/2; -lencz/2] R_demo(1:3,1:3,j)*[lencx; lency/2; -lencz/2] ...
            R_demo(1:3,1:3,j)*[0; lency/2; -lencz/2] R_demo(1:3,1:3,j)*[0; -lency/2; lencz/2] ...
            R_demo(1:3,1:3,j)*[lencx; -lency/2; lencz/2] R_demo(1:3,1:3,j)*[lencx; lency/2; lencz/2] ...
            R_demo(1:3,1:3,j)*[0; lency/2; lencz/2] ]';
        vm=vm(:,[a b c]);
        if j==1
