function plot_rigidbody_traj_sven1(R_FS_demo,R_FS_recon,R_demo,p_obj_demo,R_recon,p_obj_recon,constraints,title)
...
inc = 6; % how many rigid bodies drawn along trajectory %5 for pouring, 8 for sine, 4 for obstacle avoidance

lencx = 0.05*scale; % length of cube
lency = 0.07*scale; % length of cube
lencz = 0.15*scale; % length of cube
...
Rx_FS_demo = []; Ry_FS_demo = []; Rz_FS_demo = [];
...
% arrow3(p_demo,p_demo+len*Rx_FS_demo,['_y' linewidth],t,2*t)
% arrow3(p_demo,p_demo+len*Ry_FS_demo,['_m' linewidth],t,2*t)
% arrow3(p_demo,p_demo+len*Rz_FS_demo,['_c' linewidth],t,2*t)

if ~isempty(R_recon)
    % Draw orientation rigid body reconstruction
    Rx_recon = []; Ry_recon = []; Rz_recon = []; p_recon = [];
    Rx_FS_recon = []; Ry_FS_recon = []; Rz_FS_recon = [];
    for j=round(linspace(1,N,inc))
...
%     arrow3(p_recon,p_recon+len*Rx_FS_recon,['_y' linewidth],t,2*t)
%     arrow3(p_recon,p_recon+len*Ry_FS_recon,['_m' linewidth],t,2*t)
%     arrow3(p_recon,p_recon+len*Rz_FS_recon,['_c' linewidth],t,2*t)
end
