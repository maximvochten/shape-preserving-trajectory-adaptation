...
panda:getProperty('cutoff_frequency'):set(5)

panda:getProperty('gripper_connected'):set(false)
...
wrench_limits = rtt.Variable("array")
wrench_limits:fromtab({100.0, 100.0, 100.0, 25.0, 25.0, 25.0})
panda:getProperty('upper_force_thresholds_acceleration'):set(wrench_limits)
panda:getProperty('upper_force_thresholds_nominal'):set(wrench_limits)

torque_limits = rtt.Variable("array")
torque_limits:fromtab({40,40,36,36,32,28,24})
panda:getProperty('upper_torque_thresholds_acceleration'):set(torque_limits)
panda:getProperty('upper_torque_thresholds_nominal'):set(torque_limits)
